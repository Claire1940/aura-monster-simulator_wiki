...
function matchScore(queryText: string, article: ArticleWithType, extraKeywords?: string[]): number {
  ...
  // Exact phrase match in title (stripped of "Aura Monster Simulator")
  const strippedQuery = normalizedQuery.replace(/aura monster simulator\s*/g, '').trim()
  const strippedTitle = normalizedTitle.replace(/aura monster simulator\s*/g, '').trim()
